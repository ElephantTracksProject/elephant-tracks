...
export EXECJAR=$ETDIR/../heaponly-0.1-all.jar
# Sequential add and then sequential delete
export TRACEFILE=heaponly.trace
export NAMESFILE=heaponly.names
...
exec_string="LD_LIBRARY_PATH=${ETDIR}:${LD_LIBRARY_PATH} CLASSPATH=.:${ASMJAR}:${ETDIR}:${ETDIR}/*  ${JAVA_PATH}/bin/java  -DETJAR=${ETCLASSJAR}  -classpath .:${ASMJAR}:${ETDIR}  -Xbootclasspath/a:${ETDIR}  -agentlib:ElephantTracks=:=@traceFile=${TRACEFILE}@namesFile=${NAMESFILE}@-XechoArgs@optionsFile=./etconfig@-Xdefault=OW -jar ${EXECJAR} 149"
